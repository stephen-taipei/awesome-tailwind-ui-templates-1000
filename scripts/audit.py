...
def inspect(root: Path = ROOT) -> dict:
    ...
    warnings = []
    scripts = []
    paths = template_paths(root) + [root / name for name in ('index.html', 'catalog.html', 'preview.html') if (root / name).exists()]
    ...
    for path in paths:
        ...
        label_for = {a.get('for') for tag, a, _ in tags if tag == 'label'}
        for tag, attrs, line in tags:
            ...
            if tag in ('input', 'select', 'textarea') and attrs.get('type') not in ('hidden', 'submit', 'button', 'reset'):
                if not (attrs.get('aria-label') or attrs.get('aria-labelledby') or attrs.get('id') in label_for or attrs.get('_implicit_label')):
                    issue(warnings, path, line, 'control-label-review', 'Review accessible name (implicit wrapping labels require browser checks)')
    ...
    try:
        ...
        if len(entries) != data['total'] + 2: issue(errors, sitemap, 1, 'sitemap-count', 'Sitemap does not cover the catalog')
    ...
    return {'schemaVersion': 1, 'htmlFiles': len(paths), 'templates': data['total'], 'categories': len(data['categories']),
            'inlineScriptsParsed': len(scripts), 'errors': errors, 'warnings': warnings,
